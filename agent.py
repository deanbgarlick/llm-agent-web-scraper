...
from utils.chat_utils import chat_completion_request, memory_optimise


def call_agent(prompt: str, system_prompt: str, tools_schema: List[Dict], tools_map: Dict, plan: bool = False) -> str:
    """
    ...
    # Initialize conversation with planning if requested
    if plan:
        ...
        if current_choice.finish_reason == "tool_calls":
            ...
            for tool_call in tool_calls:
                ...
                try:
                    ...
                    result = tools_map[function.name](**args)
                    tool_message = {
                        "role": "tool",
